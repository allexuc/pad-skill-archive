...
(function () {
  ...
  // タッチで入力欄に自動フォーカスすると、ソフトキーボードがせり上がって
  // 肝心の選択肢が隠れてしまう。端末で決め打ちせず、その操作が指だったかで判断する。
  // タッチ対応ノートPCのようにマウスと指を併用する端末でも正しく振る舞う。
  // any- を使うのは主入力ではなく「マウス等が繋がっているか」を見たいから。
  // hover/pointer だとタッチ対応PCが主入力=指と判定され、マウス操作でも指扱いになる。
  var HAS_KEYBOARD = !window.matchMedia ||
    window.matchMedia("(any-hover: hover) and (any-pointer: fine)").matches;

  // 指で操作されたなら true。2つの根拠を併用する。
  //  1. そもそも指しか使えない端末（スマホ・タブレット）なら常に true
  //  2. マウスと指を併用できる端末なら、その操作の pointerType で決める
  // 1 を先に見るのは、環境によって合成クリックの pointerType が "mouse" に
  // なることがあり、それでスマホにキーボードが出るのを防ぐため。
  ...
  // 絞り込むと結果を先頭から描き直すので、深くスクロールしていた場合だけ
  // 結果の先頭まで引き戻す。チップは結果より上にあるので、そこを見ている間は動かさない。
  function keepScrollInResults() {
    var y = window.pageYOffset;
    var top = el.viewport.getBoundingClientRect().top + y;
    var head = document.querySelector(".masthead");
    var stuck = head ? head.getBoundingClientRect().height : 0;
    var limit = Math.max(0, Math.round(top - stuck));
  ...
  document.addEventListener("keydown", function (e) {
    if (e.key === "/" && HAS_KEYBOARD && document.activeElement !== el.q) {
      e.preventDefault();
      el.q.focus();
    }
  });
